...
async function displayAvailableSurveys() {
  try {
    ...
    // And modify it to sort the surveys by their _id (MongoDB ObjectId contains a timestamp):
    const availableSurveys = surveys
      .filter((survey) => !submittedSurveyIds.has(survey._id))
      .sort((a, b) => {
        // Sort by numeric index extracted from _id to ensure proper ordering
...
window.submitSurvey = async (event, surveyId) => {
  ...
  try {
    ...
    if (response.ok) {
      ...
      // Update the activeIndex before refreshing
      const currentIndex = Number.parseInt(localStorage.getItem("activeSurveyIndex") || "0")
      const nextIndex = currentIndex + 1
      localStorage.setItem("activeSurveyIndex", nextIndex.toString())
